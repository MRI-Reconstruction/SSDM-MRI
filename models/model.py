    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if torch.cuda.device_count() > 1:
                    self.output, self.visuals = self.netG.module.restoration(self.cond_image,sample_num=self.sample_num,mask=self.mask,sub_kspace=self.sub_kspace)
                else:
                    self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num,mask=self.mask,sub_kspace=self.sub_kspace)
